<?php
    echo view('includes/header-labour',$patient, $eco, $preo, $posto, $follo, $proccheck, $feedcheck, $focus, $allcheck, $old_check, $old_check);    
?>                	 	
...
<script>
	$('#consent-form').submit(function(e){
		...
		$.ajax({
			type : "POST",
			data : formData,
			url : "<?php echo base_url("e-mail-labour") ?>",
			contentType: false,
			processData: false,
			success:function(response){
                ...
                if(response.result==1){
					...
					window.location = '<?php echo base_url("labour/E_consentView")?>?id='+response.msg;                  
                }
                else {
