...
<div class="col-sm-9" id="reports-pdf">
			<h3 class="mt-4 pt-2">Procedure Outcomes - Procedure Success</h3>
			...
			<div class="row">
				<div class=" col-sm-8 table-responsive" id="demo-table">
					...
					<table class="table table-bordered" style="height: 200px; width: 100%" id="mytable">
						...
						<tbody>
							...
							<?php foreach($success_status as $row){
							?>
							<tr>
							...
							<td><p>
							<?php $number = (($row['sell']/$total)*100);
									echo number_format((float)$number, 2, '.', '')."%";?></p>
							</td>
							</tr>
							<?php
							}
				...
				<div class="col-sm-7">
					<table class="table table-bordered" style="height: 200px; width: 100%" id="mytable">
						...
						<tbody>
							...
							<?php foreach($all_success as $row){
							?>
							<tr>
							<td id="report-td-bg"><p>
							<?php echo $row['name']; ?></p>
							...
							<td><p>
							<?php echo $row['complete']; ?></p>
							</td>
							<td><p>
							<?php echo $row['partial']; ?></p>
							</td>
							<td><p>
							<?php echo $row['failure']; ?></p>
							</td>
							</tr>
							<?php
							}
...
function drawChart() {
	...
	var data = google.visualization.arrayToDataTable([
		['Characteristics', 'Count'],
			<?php 
				foreach ($success_status as $row){
				   echo "['".$row['day']."',".$row['sell']."],";
			} ?>
	]);
	var options = {
		title: 'Procedure Success Classification',
		curveType: 'function',
		legend: {
			position: 'top'
		}
...
var btnSave = document.getElementById('save-pdf');
	  ...
	  btnSave.addEventListener('click', function () {
		...
		html2canvas($('#reports-pdf').get(0)).then(function(canvas) {
			...
			pdfDoc.save('ProcedureSuccess.pdf');
		});
